...
const Banner = () => {
  ...
  return (
    <section className="relative flex flex-col items-center justify-center py-20 w-full">
      {/* Floating Icons */}
      ...
      {/* Pill label */}
      <span className="mb-4 px-6 py-2 rounded-full bg-white/10 text-white/80 text-sm font-medium tracking-wide shadow backdrop-blur-md border border-white/10 z-10">
        Web3 Event Platform
      </span>
      {/* Headline */}
      <h1 className="text-[48px] md:text-[64px] font-extrabold text-center text-white leading-tight drop-shadow-lg z-10">
        Reinventing Events <br /> for the Web3 Generation
      </h1>
     
      {/* Search Section */}
      <form onSubmit={handleSearch} className="w-full flex justify-center mt-8 mb-8 z-10">
        <div className="flex items-center w-full max-w-xl bg-white/10 backdrop-blur-md rounded-full px-6 py-4 shadow-lg border border-white/10">
          <SearchNormal color="#845EC2" size={20} className="mr-3" />
          <input
            type="text"
            placeholder="Search for events, locations, or categories..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="flex-1 bg-transparent outline-none text-white text-lg placeholder-white/60"
          />
          <button type="submit" className="ml-4 px-6 py-2 rounded-full bg-[#845EC2] text-white font-semibold shadow hover:bg-[#6d4fa1] transition-colors">Search</button>
        </div>
      </form>
      ...
      <div className="mb-8 z-10">
        <Link href="/explore">
          <button className="px-8 py-3 rounded-full bg-white/10 backdrop-blur-md text-white font-semibold shadow-lg border border-white/20 hover:bg-white/20 transition-all duration-300 hover:scale-105">
            Browse All Events
          </button>
        </Link>
      </div>

     <div className='w-full h-full flex justify-center items-center py-10 relative'>
        {/* Glassy floating dots */}
        ...
        <img
          ...
          alt="Event Preview"
          className={`w-[97%] md:w-[63%] lg:w-[47%] h-[60vh] object-cover rounded-3xl transition-opacity duration-500 ${fade ? 'opacity-100' : 'opacity-0'}`}
        />
        {/* Glassmorphic overlay */}
        <div className="absolute top-5 left-1/2 -translate-x-1/2 w-full md:w-2/3 lg:w-1/2 h-[65vh] rounded-3xl pointer-events-none -z-10 border border-white/10" style={{background: 'rgba(255,255,255,0.08)', backdropFilter: 'blur(8px)'}} />
     </div>
      <style jsx>{`
